...
# --- CODE CŨ CỦA TÙNG (Comment lại) ---
        # with open(model_path, "rb") as file:
        #     bundle = pickle.load(file)

        # --- CODE MỚI: MÓC NỐI MLFLOW ---
        mlflow.set_tracking_uri("http://localhost:5000")
        print("🚀 Đang kéo model BPR_Recommender (Production) từ MLflow...")
        loaded_model = mlflow.sklearn.load_model("models:/BPR_Recommender@Production")

        
        # Hàm của Tùng cần một Dictionary, nên ta chuyển model MLflow về dạng bundle
        bundle = loaded_model.to_bundle() 
        if self.model_info:
                    self.model_info.name = "BPR_Recommender (MLflow)"
