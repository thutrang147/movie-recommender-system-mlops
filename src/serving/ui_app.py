...
def get_movie_info(movie_id: int, movies_df: pd.DataFrame) -> Dict:
    ...
    # Kiểm tra xem cột đúng là gì (thường là movieId hoặc id)
    col_name = "movie_id" if "movie_id" in movies_df.columns else ("movieId" if "movieId" in movies_df.columns else movies_df.columns[0])
    movie = movies_df[movies_df[col_name] == movie_id]
    if movie.empty:
