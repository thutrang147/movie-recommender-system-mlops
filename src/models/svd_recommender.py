...
class RobustSparseSVD:
    ...
    def evaluate(self, test_df):
        ...
        y_pred = [self.predict(row.UserID, row.MovieID) for row in test_df.itertuples()]
        
        rmse = np.sqrt(mean_squared_error(y_true, y_pred))
        mae = mean_absolute_error(y_true, y_pred)
        return rmse, mae
    def evaluate_ranking(self, train_df, test_df, top_k=10, relevance_threshold=4.0):
        """Đánh giá theo chuẩn nghiệp vụ: Recall@K, MAP@K, Coverage"""
        print(f"   [Metrics] Đang tính toán Top-{top_k} Recommendation Metrics...")
        
        # 1. Lọc các phim "thực sự liên quan" (rating >= 4.0) trong tập Test của mỗi User
        relevant_items = test_df[test_df['Rating'] >= relevance_threshold]
        test_user_items = relevant_items.groupby('UserID')['MovieID'].apply(set).to_dict()
        
        # Nhóm các phim User đã xem trong tập Train để loại trừ khỏi danh sách gợi ý
        ...
        # 2. Xây dựng ma trận Item Bias một lần để tăng tốc độ tính toán
        item_biases = np.zeros(len(self.item_map))
        for i_id, i_idx in self.item_map.items():
            item_biases[i_idx] = self.item_bias.get(i_id, 0)

        # Đảo ngược mapping để truy xuất ID gốc nhanh hơn
        reverse_item_map = {idx: i_id for i_id, idx in self.item_map.items()}

        # Chỉ đánh giá những User có xuất hiện trong cả Train và có phim "thích" trong Test
        eval_users = [u for u in test_user_items.keys() if u in self.user_map]
        
        for user_id in eval_users:
            u_idx = self.user_map[user_id]
            
            # Dự đoán siêu tốc cho TẤT CẢ items cùng lúc (Vectorization)
            ...
            # Đánh dấu các phim đã xem trong Train thành -vô cực để không gợi ý lại
            ...
            top_k_items = [reverse_item_map[idx] for idx in top_k_indices]
            
            recommended_items_pool.update(top_k_items)
            
            # Tính toán Hits, Recall, Average Precision
            actual_relevant = test_user_items[user_id]
            hits = 0
            sum_precs = 0
            ...
            # Recall@K: Tìm được bao nhiêu phim relevant trong Top K / Tổng số phim relevant của user đó
            recall = hits / len(actual_relevant) if actual_relevant else 0
            # Average Precision@K
            ap = sum_precs / min(len(actual_relevant), top_k)
        ...
        map_k = np.mean(aps)
        # Coverage: Tỷ lệ phim được hệ thống gợi ý ít nhất 1 lần trên tổng số phim trong catalog
        coverage = len(recommended_items_pool) / len(self.item_map)
